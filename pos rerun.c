...
task main()
{
	...
	while (true)
	{
	  ...
	  //So to fix this the power is fixed and the Joysticks just determine direction. You can change the power of the motors
		...
		if (fwdNotCrab == true){
		if (vexRT[leftJoy] >= 10 && vexRT[rightJoy] >= 10)//Adds a thershhold of +10 to set values
		{
			...
			forwardVal = (vexRT(Ch2)) + (vexRT(Ch3))/2 ; //sets + joy values to varibles
		}
		//seeing if the joy is being pushed in the negative y direction, meaning the bot should go backwards
		else if (vexRT[leftJoy] <= -10 && vexRT[rightJoy] <= -10)//Adds a thershhold of -10 to set values
		{
			fwdJoyTog = true;
			bwd = true;
			forwardVal = (vexRT(Ch2)) + (vexRT(Ch3))/2; //sets - joy values to varibles
		...
		else
		{
			...
			if (crabJoyTog == false)//if there is no input to the crabJoy,
			{
				...
				{
					SensorValue[lft] = 0;
					SensorValue[gyro] = 0;//clear the encoder on the front wheel(which is used for all encoder values)
			...
			//and that value will get printed out using the code below. And the cycle continues indefinatly.
					...
					if (fwd == true)// meaning the bot went forward
					{
						...
						writeDebugStreamLine("reset(lef);");
	...
	//concept.
		...
		if (vexRT[rightJoy] <= -10 && vexRT[leftJoy] >= 10)
		{
			spinJoyTog = true;
			rightSpin = true;
			spinVal = (vexRT(Ch3)) - (vexRT(Ch2))/2; //sets + joy values to varibles
		}
		else if(vexRT[rightJoy] >= 10 && vexRT[leftJoy] <= -10)
		{
			spinJoyTog = true;
			leftSpin = true;
			spinVal = -(vexRT(Ch2)) - (vexRT(Ch3))/2;
		}
		else
		{
			if (spinJoyTog == false)
															...
															//spin function in MechWheelFunctions.c
			{
				if(fwdJoyTog == false)
				{
					...
					if (rightSpin == true)// meaning the bot went right
					{
						...
						writeDebugStream(encoderVal);
						writeDebugStreamLine(");");
						writeDebugStreamLine("reset(giro);");
						rightSpin = false;
						spinJoyTog = false;
					}else{
						sprintf(encoderVal, "%d", abs(SensorValue[gyro]));
		...
		else if (vexRT[Btn6U]== 0 && vexRT[Btn6D]== 1  )//Checks for single button press
		{
			...
			if(armTog == false)
			{
				...
				if(upArm == true)
				{		sprintf(oencoderVal, "%d", abs(SensorValue[lif]));
						sprintf(encoderVal, "%d", abs(SensorValue[ritlif]));
						writeDebugStream("lift(U,");
						writeDebugStream(encoderVal);
						writeDebugStream(",");
						writeDebugStream(oencoderVal);
						writeDebugStreamLine(");");
						upArm = false;
						armTog = false;
					}else{
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		else if (vexRT[Btn5U]== 1 && vexRT[Btn5D]== 1  )//Checks for single button press
		{
			close = true;
			intakeTog = true;
			intakeOut = true;
